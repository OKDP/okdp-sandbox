...
You can use the native## Verification Example
...
3. **Test with cURL**:
   ...
   curl -skL -X POST https://trino-trino.okdp.sandbox/v1/statement \
...
4. **Test with Trino CLI**:
   ...
   trino \
     ...
     --insecure
   ```
oordinator. Use `localhost` if properly forwarded or running locally.
...
curl -sk \
  ...
  https://<SERVICE_PREFIX>.okdp.sandbox/v1/statement
```
